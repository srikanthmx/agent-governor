...
export const localToolCatalog: LocalToolCatalogItem[] = [
  ...
  { id: "ollama", label: "Ollama", kind: "agent", runnable: true, commandCandidates: ["ollama"], capabilities: ["requirements", "design", "review"], installUrl: "https://ollama.com/download", setupCommand: "ollama pull llama3.1", notes: "Local model runtime exposed through Ollama CLI and OpenAI-compatible API.", marketRank: 4, marketSummary: "Best local fallback for private, low-cost model work when a suitable tool-calling model is installed.", researchSources: ["Ollama documentation"] },
  { id: "opencode", label: "OpenCode", kind: "agent", runnable: true, commandCandidates: ["opencode"], capabilities: ["requirements", "design", "implementation", "review"], installCommand: "curl -fsSL https://opencode.ai/install | bash", installUrl: "https://opencode.ai/docs/", setupCommand: "opencode auth login", notes: "Open source terminal coding agent with `opencode run`.", marketRank: 5, marketSummary: "Open source CLI agent with a configurable command surface.", researchSources: ["OpenCode docs"] },
  { id: "aider", label: "Aider", kind: "agent", runnable: true, commandCandidates: ["aider"], capabilities: ["implementation", "review"], installCommand: "python -m pip install aider-install && aider-install", installUrl: "https://aider.chat/docs/install.html", setupCommand: "aider", notes: "Open source CLI pair programmer.", marketRank: 6, marketSummary: "Mature pair-programming CLI, best for focused implementation and review loops.", researchSources: ["Aider install docs"] },
  { id: "goose", label: "Goose", kind: "agent", runnable: true, commandCandidates: ["goose"], capabilities: ["requirements", "design", "implementation", "review"], installCommand: "brew install block-goose-cli", installUrl: "https://block.github.io/goose/docs/getting-started/installation/", setupCommand: "goose configure", notes: "Local extensible coding agent from Block.", marketRank: 7, marketSummary: "Extensible local agent worth tracking for MCP-heavy workflows.", researchSources: ["Goose installation docs"] },
  { id: "qwen-code", label: "Qwen Code", kind: "agent", runnable: true, commandCandidates: ["qwen", "qwen-code"], capabilities: ["requirements", "design", "implementation", "review"], installCommand: "npm install -g @qwen-code/qwen-code", installUrl: "https://github.com/QwenLM/qwen-code", setupCommand: "qwen", notes: "Terminal coding agent in the Qwen ecosystem.", marketRank: 8, marketSummary: "Emerging CLI option for Qwen-backed coding workflows.", researchSources: ["Qwen Code repository"] },
  { id: "amp", label: "Amp", kind: "agent", runnable: true, commandCandidates: ["amp"], capabilities: ["requirements", "design", "implementation", "review"], installUrl: "https://ampcode.com/", setupCommand: "amp", notes: "CLI coding agent; install details may vary by account.", marketRank: 9, marketSummary: "Commercial CLI coding agent to evaluate when account access is available.", researchSources: ["Amp product site"] },
  { id: "amazon-q", label: "Amazon Q Developer CLI", kind: "agent", runnable: true, commandCandidates: ["q"], capabilities: ["requirements", "design", "implementation", "review"], installUrl: "https://docs.aws.amazon.com/amazonq/latest/qdeveloper-ug/command-line.html", setupCommand: "q login", notes: "AWS local CLI coding assistant.", marketRank: 10, marketSummary: "Best fit for AWS-heavy local development environments.", researchSources: ["Amazon Q Developer CLI docs"] },
  { id: "openhands", label: "OpenHands", kind: "agent", runnable: true, commandCandidates: ["openhands"], capabilities: ["requirements", "design", "implementation", "review"], installUrl: "https://docs.all-hands.dev/", notes: "Open source software engineering agent; often run via Docker or server mode.", marketRank: 11, marketSummary: "Research target for server/Docker agent workflows rather than simple CLI routing.", researchSources: ["OpenHands docs"] },
...
function promptCapabilityCheck(id: string, command: string): { ok: boolean; reason: string } {
  ...
  if (["claude", "gemini", "opencode", "aider", "goose", "qwen-code", "ollama"].includes(id)) {
    const result = commandOutput(command, ["--help"]);
    return {
      ok: result.ok,
