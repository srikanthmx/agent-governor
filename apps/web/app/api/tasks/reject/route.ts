...
export async function POST(request: Request) {
  ...
  const args = ["agent", "reject", taskId];
  if (body.stage) args.push("--stage", body.stage);
  args.push("--owner", owner);
  if (body.comment) args.push("--comment", body.comment);
  ...
  return NextResponse.json(
    { ok: result.exitCode === 0, output, error: result.exitCode === 0 ? undefined : output || `reject exited with ${result.exitCode}` },
